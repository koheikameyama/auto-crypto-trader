...
import {
  runMultiSignalBacktest,
  type MultiSignalParams,
  type MsResult,
} from "../backtest/multi-signal-regime-engine.js";

export interface MsWfArgs {
  btcBars: DailyBar[];
  onchainBars: OnchainBar[];
  dxyBars: DailyBar[];
  vixBars: DailyBar[];
  paramGrid: { [K in keyof MultiSignalParams]?: number[] };
  fixed?: Partial<MultiSignalParams>;
  isDays: number;
  oosDays: number;
  stepDays: number;
...
export interface MsWfWindow {
  windowIndex: number;
  isStart: Date;
  isEnd: Date;
  oosStart: Date;
  oosEnd: Date;
  bestParams: MultiSignalParams;
  isSharpe: number;
  oosSharpe: number;
  oosMar: number;
  oosPf: number;
...
export interface MsWfAggregate {
  windows: MsWfWindow[];
  oosAvgSharpe: number;
  oosAvgMar: number;
  oosAvgPf: number;
...
function cartesian(
  grid: { [K in keyof MultiSignalParams]?: number[] },
  fixed: Partial<MultiSignalParams>,
): MultiSignalParams[] {
  const keys = Object.keys(grid) as (keyof MultiSignalParams)[];
  let combos: Array<Partial<MultiSignalParams>> = [{ ...fixed }];
  for (const k of keys) {
    const values = grid[k]!;
    const next: Array<Partial<MultiSignalParams>> = [];
    for (const c of combos) for (const v of values) next.push({ ...c, [k]: v });
    combos = next;
  }
  return combos as MultiSignalParams[];
}
...
function optimizeOnIs(
  ...
  dxyBars: DailyBar[],
  vixBars: DailyBar[],
  combos: MultiSignalParams[],
  initialCapital: number,
): { bestParams: MultiSignalParams; bestSharpe: number } {
  const all: Array<{ params: MultiSignalParams; result: MsResult }> = [];
  for (const params of combos) {
    const r = runMultiSignalBacktest({
      btcBars,
      onchainBars,
      dxyBars,
      vixBars,
...
export function runMsWalkForward(args: MsWfArgs): MsWfAggregate {
  const {
    btcBars,
    onchainBars,
    dxyBars,
  ...
  for (
    ...
    // Pass full history up to window end for feature warm-up (like R4c)
    const isOn = sliceWindow(onchainBars, onchainBars[0].date, isEnd);
    const oosOn = sliceWindow(onchainBars, onchainBars[0].date, oosEnd);
    const isDxy = sliceWindow(dxyBars, dxyBars[0]?.date ?? isStart, isEnd);
    const oosDxy = sliceWindow(dxyBars, dxyBars[0]?.date ?? oosStart, oosEnd);
    const isVix = sliceWindow(vixBars, vixBars[0]?.date ?? isStart, isEnd);
    const oosVix = sliceWindow(vixBars, vixBars[0]?.date ?? oosStart, oosEnd);

    const opt = optimizeOnIs(
      isBtc,
      isOn,
      isDxy,
      isVix,
    ...
    const oos = runMultiSignalBacktest({
      btcBars: oosBtc,
      onchainBars: oosOn,
      dxyBars: oosDxy,
      vixBars: oosVix,
