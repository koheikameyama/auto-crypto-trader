...
async function main() {
  ...
  try {
    ...
    const start = dayjs("2019-11-01"); // ETH funding starts here
    const end = dayjs();

    const eth = await loadBars(prisma, "ETH-USD", start.toDate(), end.toDate());
    // onchain BTC (used as placeholder; wOnchain=0 so unused)
    ...
    const ethFunding = await loadFundingBars(prisma, "ETHUSDT", start.toDate(), end.toDate());

    console.log(`\n========================================`);
    console.log(`Step 2: Scheme E on ETH (cross-asset validation)`);
    console.log(`Weights: wDxy=0.60, wFunding=0.40 (all others = 0)`);
    console.log(`Period: ${start.format("YYYY-MM-DD")} -> ${end.format("YYYY-MM-DD")}`);
    console.log(`ETH ${eth.length} / DXY ${dxy.length} / VIX ${vix.length} / TNX ${tnx.length} / Funding(ETHUSDT) ${ethFunding.length}`);
    console.log(`========================================\n`);

    // Small hyper-grid (same as BTC Scheme E optimization)
    const hyperGrid = [
    ...
    const firstDate = eth[0].date;
    const lastDate = eth[eth.length - 1].date;
    ...
    for (let startDay = 0; startDay + isDays + oosDays <= totalDays; startDay += stepDays) {
      ...
      const isEth = sliceWindow(eth, isStart, isEnd);
      const oosEth = sliceWindow(eth, oosStart, oosEnd);
      if (isEth.length === 0 || oosEth.length === 0) { idx++; continue; }
      ...
      const isFund = sliceWindow(ethFunding, ethFunding[0]?.date ?? isStart, isEnd);
      const oosFund = sliceWindow(ethFunding, ethFunding[0]?.date ?? oosStart, oosEnd);
      ...
      for (const hp of hyperGrid) {
        ...
        const r = runWeightedBacktest({
          asset: "ETH-USD",
          btcBars: isEth, onchainBars: isOn, dxyBars: isDxy, vixBars: isVix,
      ...
      const oos = runWeightedBacktest({
        asset: "ETH-USD",
        btcBars: oosEth, onchainBars: oosOn, dxyBars: oosDxy, vixBars: oosVix,
      ...
      windows.push({
        idx,
        isStart: isEth[0].date,
        isEnd: isEth[isEth.length - 1].date,
        oosStart: oosEth[0].date,
        oosEnd: oosEth[oosEth.length - 1].date,
    ...
    if (bh) {
      ...
      console.log(`Beats ETH BH: ${oosAvgSharpe > bh.sharpe ? "YES ★" : "NO"} (${oosAvgSharpe.toFixed(3)} vs ${bh.sharpe.toFixed(3)})`);
    }
    ...
    const filepath = path.join(outDir, `scheme-e-eth-${ts}.md`);
    const lines: string[] = [
      ...
      `|---|---|---|---|---|`,
      `| ETH BH | ${bh?.sharpe.toFixed(3)} | ${((bh?.maxDrawdown ?? 0) * 100).toFixed(2)}% | — | baseline |`,
      `| **Scheme E on ETH** | **${oosAvgSharpe.toFixed(3)}** | **${(oosMaxDd * 100).toFixed(2)}%** | **${(drop * 100).toFixed(1)}%** | **${bh && oosAvgSharpe > bh.sharpe ? "YES ★" : "NO"}** |`,
      ``,
      `## OOS KPIs`,
      ``,
      `| Metric | Value |`,
